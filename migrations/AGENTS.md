...
| `0005_telegram_dm_link.sql` | Forward: `telegram_link_codes` (user-scoped one-time codes, UNIQUE `code_hash`, FK `users` CASCADE, partial unique index one-unused-per-user on `user_id` WHERE `used_at IS NULL`) + `user_telegram_links` (PK `user_id`, UNIQUE `telegram_user_id`, FK `users` CASCADE). No `empresa_id` — link is global per user. |
