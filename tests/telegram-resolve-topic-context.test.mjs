 ...
 */
test("lt-resolve-happy-path: chat -1001 thread 55 → {empresa_id, expert_id}", () => {
  const db = openDb();
  ...
  const result = resolveTelegramTopicContext(db, "-1001", "55");
 ...
 */
test("lt-resolve-unknown-thread-null: thread 999 with maps only for 55 → null", () => {
  const db = openDb();
  ...
  const result = resolveTelegramTopicContext(db, "-1001", "999");
 ...
 */
test("lt-resolve-soft-deleted-expert-null: experts.deleted_at set → null", () => {
  const db = openDb();
 ...
 */
test("lt-resolve-soft-deleted-empresa-null: empresas.deleted_at set → null", () => {
  const db = openDb();
 ...
 */
test("lt-resolve-join-requires-matching-chat: topic chat_id ≠ empresa chat_id → null", () => {
  const db = openDb();
  ...
  const resultDriftedChat = resolveTelegramTopicContext(db, "-9999", "55");
