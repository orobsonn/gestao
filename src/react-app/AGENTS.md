...
- **Hermetic locks:** `tests/web-shell-*.test.mjs` / `tests/web-home-*.test.mjs` / `tests/web-domain-*.test.mjs` via `node --experimental-strip-types --test`. Relative imports in pure libs use `.ts` suffix for Node ESM.
