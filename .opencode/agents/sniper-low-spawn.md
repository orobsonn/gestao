...
Use Edit. Change only what `fix_hint` prescribes. Preserve surrounding code exactly. LOW defects (typo, naming, dead code, cosmetic) are trivial — resist the urge to "improve" anything beyond the named line. A cosmetic fix that touches behavior is out of scope: if the change is more than skin-deep, emit BLOCKED instead of guessing.
...
```
opencode run --agent sniper-low-spawn ...
```

Do **not** use `opencode run --agent sniper-low` — that file is `mode: subagent` and OC falls back to default primary (probe P2).
In-session loop dispatches `task(subagent_type: "sniper-low")` against the subagent twin.
