...
```
opencode run --agent test-author-spawn ...
```

Do **not** use `opencode run --agent test-author` — that file is `mode: subagent` and OC falls back to default primary (probe P2).
In-session loop dispatches `task(subagent_type: "test-author")` against the subagent twin.
