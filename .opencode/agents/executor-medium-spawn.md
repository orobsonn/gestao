...
```
opencode run --agent executor-medium-spawn ...
```

Do **not** use `opencode run --agent executor-medium` — that file is `mode: subagent` and OC falls back to default primary (probe P2).
In-session loop dispatches `task(subagent_type: "executor-medium")` against the subagent twin.
