/** @description OC version-check plugin — advisory only, no blocking. Catalog health + harness staleness. */
...
/**
 * @description Delivers an advisory message: TUI toast first, console/deps.warn fallback.
 * Never throws — a delivery failure degrades to a log line instead of blocking bootstrap.
 */
async function deliverAdvisory(
  message: string,
  client: Pick<PluginInput, "client">["client"] | undefined,
  deps: Pick<VersionCheckDeps, "warn" | "toastTimeoutMs">,
): Promise<void> {
  try {
...
/**
 * @description OpenCode plugin — advisory catalog + harness-staleness checks (fail-open).
 */
export async function createVersionCheck(
  ...
  if (catalogMessage) await deliverAdvisory(catalogMessage, client, deps)

  try {
    const staleMessage = checkHarnessVersionStale(projectRoot, deps)
    if (staleMessage) await deliverAdvisory(staleMessage, client, deps)
  } catch {
